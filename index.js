...
function decodeSheet() {
    ...
    for (let i = 1; i < sheetData.values.length; i++) {
        let newSheet = sheetData.values[i];
        for (let j = 0; j < 20 - sheetData.values[i].length; j++) {
            newSheet.push('')
        }
        nsheet.push(newSheet)
    }

    let lines = [];
    for (let j = 0; j < nsheet.length; j++) {
        let ymm = nsheet[j][3] ? nsheet[j][3].replace(/\n/g,`	`).replace('"','').split(`	`) : nsheet[j][3];
        ymms = [];
        if (ymm) {
            for (let k = 0; k < ymm.length; k++) {
                ...
                for (let p = 0; p < ymm[k].length; p++) {
                    if ((ymm[k].charAt(p) == ' ' && mode !== 4) || (ymm[k].charAt(p) == '-' && mode == 1)) {
                        ...
                        if (mode == 4) {
                            if (ymm[k].charAt(p) !== '"' && ymm[k].charCodeAt(p) !== 13)
        ...
        //Find Resale Value
        let cost = Number(nsheet[j][12])*2;
        let retail = cost;
        if (cost < 45) retail = 45;
        else if (cost < 95) retail = 95;
        else retail = Math.ceil(cost/10)*10;

        if (nsheet[j][18] == 'key' && !nsheet[j][6].toLowerCase().includes('pt')) {
        ...
        let obj = {
            car: nsheet[j][0],
            col: nsheet[j][1],
            row: Number(nsheet[j][2]),
            ymm: ymms,
            fccid: nsheet[j][4],
            model: nsheet[j][5],
            type: nsheet[j][6],
            amount: Number(nsheet[j][7]),
            frequency: nsheet[j][8],
            chip: nsheet[j][9],
            battery: nsheet[j][10],
            link: nsheet[j][11],
            price: Number(nsheet[j][12]),
            notes: nsheet[j][13],
            imgLink: nsheet[j][14],
            old: nsheet[j][15],
            xhorse: nsheet[j][16],
            id: nsheet[j][17],
            itemType: nsheet[j][18],
            bladeType: nsheet[j][19],
            retail: retail,
        }
        lines.push(obj);
    }
...
let carObj, carMakes, carMakesList;
let dataMakes = $('makes');
